jQuery(document).ready(function ($) {
	...
	$("button#add-me").click(function () {
		...
		const specialCharges = {
			"Fish/Medicine": 10, // Special charge for this category (Fish/Meat/Medicine)
			"Documents/Electronics": 20, // Special charge for this category (Documents/Electronics)
		};
		...
		newItem.setAttribute("price", kg * price + specialCharge); // Add special charge to price
		newItem.setAttribute("unitPrice", price);
		newItem.setAttribute("specialCharge", specialCharge); // Store the special charge for later use
		newItem.innerHTML = `
        <span class="category">${category}</span>
        <span class="name">${itemName}</span>
        <span class="size">${size}KG</span>
        <span class="price">${currencySymbol}${parseFloat(
			(kg * price + specialCharge).toFixed(2)
		).toLocaleString()} (+${currencySymbol}${specialCharge})</span>
    ...
    `;
		...
		document.getElementById("total-kgs").textContent = totalKg;
		document.getElementById("special-charge-value").textContent =
			totalSpecialCharge.toFixed(2); // Update special charge value
	...
	const packerInput = document.getElementById("packerName");
	// const senderaddressInput = document.getElementById("sender-address");
	const packeraddressInput = document.getElementById("packerAddress");

	// const senderValue = document.getElementById("sendername-value");
	const packerValue = document.getElementById("packerNameValue");
	// const senderaddressValue = document.getElementById("senderaddress-value");
	const packeraddressValue = document.getElementById("packerAddressValue");
	...
	if (
		...
		packerInput.addEventListener("input", (event) => {
			packerValue.textContent = event.target.value;
			console.log("Packer Name:", event.target.value); // Debugging line
		});
		...
		packeraddressInput.addEventListener("input", (event) => {
			packeraddressValue.textContent = event.target.value;
		});
	}
