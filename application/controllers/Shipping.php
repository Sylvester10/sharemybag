...
class Shipping extends MY_Controller
{
    ...
    public function shippings_ajax()
    {
        ...
        foreach ($list as $y) {
            ...
            $sender_contact = '' . $y->sender_phone . ' <br> <br> ' . $y->sender_email . ' <br> <br> ' . $y->sender_address . '';
            $receiver_contact = '' . $y->receiver_phone . ' <br> <br> ' . $y->receiver_email . ' <br> <br> ' . $y->receiver_address . '';
    ...
    public function update_shipping($id)
    {
        ...
        $page_title = 'Update Shipping Info: ' . $shipping_details->sender_name;
    ...
    public function view_shipping($id)
    {
        ...
        $page_title = 'shipping Info: ' . $shippings_details->sender_name;
        $this->admin_header($page_title, $page_title);
    ...
    public function activate_shipping($id)
    {
        ...
        $this->session->set_flashdata('status_msg', 'shipping now available.');
    ...
    public function deactivate_shipping($id)
    {
        ...
        $this->session->set_flashdata('status_msg', 'shipping now unavailable.');
        redirect($this->agent->referrer());
    }
    ...
    public function delete_shipping($id)
    {
        ...
        $this->session->set_flashdata('status_msg', 'shipping deleted successfully.');
        redirect($this->agent->referrer());
    }
    ...
    public function bulk_actions_shipping()
    {
        ...
        if ($this->form_validation->run()) {
            if ($selected_rows > 0) {
                $this->shipping_model->bulk_actions_shipping();
            } else {
