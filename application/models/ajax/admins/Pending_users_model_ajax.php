...
class Pending_users_model_ajax extends CI_Model
{
        ...
        return form_open('admin_users/message_admin/' . $y->id, 'class="admin-form-modal__form"') .
            '<div class="modal-body admin-form-modal__body">
				<div class="admin-form-modal__section">
					<label for="pending_user_message_' . $y->id . '">Message *</label>
					<textarea class="form-control t200 w-100" id="pending_user_message_' . $y->id . '" name="message" placeholder="Your message" required></textarea>
				</div>
			</div>
    ...
    {
        $y = $this->user_read_model->get_user_details_by_id($id);
        return '<div class="modal fade admin-form-modal admin-form-modal--compact" id="message' . $id . '" role="dialog" aria-modal="true" aria-labelledby="pending_user_message_title_' . $id . '">
					<div class="modal-dialog admin-form-modal__dialog">
						<div class="modal-content modal-form admin-form-modal__content">
							<div class="modal-header ">
								...
								<h4 class="modal-title admin-form-modal__title" id="pending_user_message_title_' . $id . '">Message: ' . $y->firstname . '</h4>
							</div><!--/.modal-header-->
    ...
    private function modal_unverify($user)
    {
        ...
        return '<div class="modal fade admin-form-modal admin-form-modal--compact" id="unverify' . $id . '" role="dialog" aria-modal="true" aria-labelledby="pending_unverify_title_' . $id . '">
			<div class="modal-dialog admin-form-modal__dialog">
				<div class="modal-content modal-form admin-form-modal__content">
					<div class="modal-header ">
						<div class="pull-right">
						...
						<h4 class="modal-title admin-form-modal__title" id="pending_unverify_title_' . $id . '">Un-verify: ' . html_escape($y->firstname) . '</h4>
					</div>
					' . form_open('admin_users/unverify_user/' . $id, 'class="admin-verification-form admin-form-modal__form"') . '
					<div class="modal-body admin-form-modal__body">
						<div class="admin-form-modal__section">
							<div class="form-group">
								<label for="pending_unverify_reason_' . $id . '">Reason *</label>
								<select name="rejection_reason" id="pending_unverify_reason_' . $id . '" class="form-control" required>' . $reason_options . '</select>
							</div>
							<div class="form-group">
								<label for="pending_unverify_note_' . $id . '">Additional note</label>
								<textarea name="rejection_note" id="pending_unverify_note_' . $id . '" class="form-control" rows="4" maxlength="500" placeholder="Optional extra guidance for the user."></textarea>
							</div>
						</div>
					</div>
					<div class="modal-footer admin-form-modal__footer">
